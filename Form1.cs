...
namespace ExamenSB_U2
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                LenguajeNaturalLst.Items.Add(line);
            }
        }
        ...
        private void TraducirBtn_Click(object sender, EventArgs e)
        {
            ...
            List<string> Funciones = new List<string> { "imprimir", "leer", "mientras", "para", "si", "regresar" };
            ...
            List<string> Funcion = new List<string> { "Console.WriteLine", "Console.ReadLine", "While", "For", "If", "Return" };
            ...
            foreach (string line in LN)
            {
                Numeracion++;
                bool Aumento = false;
                if (Especiales.Any(f => line.Contains(f)))
                {
                    if (line.Contains(Especiales[0]))
                    {
                        ...
                        {
                            LenguajeNaturalLst.Items[Numeracion - 2].Text += " { ";
                            Llaves++;
                            continue;
                        }
                    }
                    ...
                    {
                        Llaves--;
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Orden.LastOrDefault()}");
                        if (Orden.Count > 0) Orden.RemoveAt(Orden.Count - 1);
                        continue;
                    }
                }
                if (AccesoCF.Any(f => line.Contains(f)))
                {
                    int AccessIndex = AccesoCF.Select((item, i) => line.Contains(item) ? i : -1).FirstOrDefault(i => i != -1);
                    if (ClaseF.Any(f => line.Contains(f)))
                    {
                        int ClassIndex = ClaseF.Select((item, i) => line.Contains(item) ? i : -1).FirstOrDefault(i => i != -1);
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves*2)}{AccessCF[AccessIndex]} {ClassF[ClassIndex]} {line.TrimStart().Split(' ')[2]}");
                    ...
                    else
                    {
                        string Traducido = Regex.Match(line, @"\((.*?)\)").Groups[1].Value != ""
                                                                                ...
                                                                                : "";
                        if (TiposDato.Any(f => line.Contains(f)))
                        {
                            // {Regex.Match(line, @"\((.*?)\)").Groups[1].Value}
                            int DatoIndex = TiposDato.Select((item, i) => line.Contains(item) ? i : -1).FirstOrDefault(i => i != -1);
                            VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{AccessCF[AccessIndex]} Function {line.TrimStart().Split(' ')[2]}({Traducido}) as {DataType[DatoIndex]} ");
                            Orden.Add($"{Specials[1]} Function");
                        }
                        else
                        {
                            VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{AccessCF[AccessIndex]} Sub {line.TrimStart().Split(' ')[1]} ");
                            Orden.Add($"{Specials[1]} Sub");
                        }
                            
                    }
                }
                else if (Funciones.Any(f => line.Contains(f)))
                {
                    ...
                    {
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]}({Regex.Match(line, @"\((.*?)\)").Groups[1].Value})");
                    }
                    ...
                    else if (FuncionIndex == 3) // For
                    {
                        ...
                        if (DivisionesFor[1].Contains('='))
                            VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]} {DivisionesFor[0]} To {Regex.Match(DivisionesFor[1], "[<>\\=]+(.+)$").Groups[1].Value.Trim()}");
                        else
                        {
                            if (DivisionesFor[2].Contains("++"))
                                VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]} {DivisionesFor[0]} To {Regex.Match(DivisionesFor[1], "[<>\\=]+(.+)$").Groups[1].Value.Trim()} - 1");
                            else
                                VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]} {DivisionesFor[0]} To {Regex.Match(DivisionesFor[1], "[<>\\=]+(.+)$").Groups[1].Value.Trim()} - 1");
                        }
                        Orden.Add("Next");
                    ...
                    {
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]} {Regex.Match(line, @"\((.*?)\)").Groups[1].Value} Then");
                        Orden.Add($"{Specials[1]} If");
                    }
                    else if (FuncionIndex == 5) // Return
                    {
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{Funcion[FuncionIndex]} {string.Join(" ", line.TrimStart().Split(' ').Skip(1))}");
                    }
                }
                else if (TiposDato.Any(f => line.Contains(f)))
                {
                    if(Variables.Contains(line.TrimStart().Split(' ')[1]))
                    {
                        ErrorEnLinea(line);
                        continue;
                    }
                    int DatoIndex = TiposDato.Select((item, i) => line.Contains(item) ? i : -1).FirstOrDefault(i => i != -1);
                    if (line.Contains('='))
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}Dim {line.TrimStart().Split(' ')[1]} As {DataType[DatoIndex]} = {string.Join(" ", line.TrimStart().Split(' ').Skip(3))}");
                    else
                        VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}Dim {line.TrimStart().Split(' ')[1]} As {DataType[DatoIndex]}");
                    Variables.Add(line.TrimStart().Split(' ')[1]);
                }
                else if (Variables.Any(f => line.TrimStart().Split(' ')[0].Contains(f)))
                {
                    VisualBasicLst.Items.Add($"{new string(' ', Llaves * 2)}{line}");
                }
                else
                {
                    VisualBasicLst.Items.Add(line);
            ...
            LenguajeNaturalLst.Items.Cast<ListViewItem>()
                                            .Where(item => Regex.IsMatch(item.Text, @"^\s*\{"))
                                            .ToList()
        ...
        public void ErrorEnLinea(string Linea)
        {
            VisualBasicLst.Items.Add(Linea);
            VisualBasicLst.Items[VisualBasicLst.Items.Count - 1].ForeColor = Color.Red;
            LineasNTraducidasLb.Text = (int.Parse(LineasNTraducidasLb.Text) + 1).ToString();
        }
